...
# USE ON E2M3A data from 2015 and 2017!!!
...
# Vector with CO2 timestamps we should keep (for 2017 data)
co2_filter <-  c("01:29","05:29","09:29","13:29","17:29","21:29",
                 "01:28","05:28","09:28","13:28","17:28","21:28",
                 "01:27","05:27","09:27","13:27","17:27","21:27",
                 "01:26","05:26","09:26","13:26","17:26","21:26",
                 "01:25","05:25","09:25","13:25","17:25","21:25",
                 "01:24","05:24","09:24","13:24","17:24","21:24",
                 "01:23","05:23","09:23","13:23","17:23","21:23",
                 "01:22","05:22","09:22","13:22","17:22","21:22",
                 "01:21","05:21","09:21","13:21","17:21","21:21",
                 "01:20","05:20","09:20","13:20","17:20","21:20",
                 "01:19","05:19","09:19","13:19","17:19","21:19",
                 "01:18","05:18","09:18","13:18","17:18","21:18",
                 "01:17","05:17","09:17","13:17","17:17","21:17")
...
max_time_ofset <- 2700 # in seconds (initial used 2700 = 45 mins, but need to increase to include more measurements)
...
#---------------------
# Remove CO2 rows which are related to zeroing (Ingunn found out we should keep CO2
# rows with certain timestamps -  these are selected above in the input params)


# Add "T" and colon to timestamp to make sure matches are only found for "hour:minute"
co2_filter <- paste("T", co2_filter,":", sep="")
...
while (data_finished==0) {
  ...
  if (state == "find_co2") {
    if (!is.na(new_df$pco2_3_CO2[count])){
      new_df$delete_col[count]<-1
      state <- "check_time_match"
    }
  } 
  
  if (state == "check_time_match") {
    
    for (i in 1:length(co2_filter)) {
      if(grepl(co2_filter[i], new_df$datetime[count])){
        new_df$delete_col[count] <- 0
      }
    }
  ...
  count <- count + 1
  if (count > nrow(new_df)) {
    data_finished <- 1
  }  
}


output_new_df <- paste("output/2.1.chosen_cols_with_deleteCol_",input_files[file_loop], sep="")
write.table(new_df, file = output_new_df, sep ="\t", row.names=FALSE)

#---
# Something change in measurement cycle after 5th og August (row nr . 
# After this point remove all rows where InstrumentZero change (from autozero plots this is not nessesary for
# measurements prior to august). This should remove most autozero values
