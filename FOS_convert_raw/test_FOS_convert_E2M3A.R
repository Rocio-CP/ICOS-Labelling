...
co2_col_2 <- "pco2_2_CO2"
co2_col_3 <- "pco2_3_CO2"

other <- c(oxy="sbe37po_oxy")         # Make this "other-varaible" to allow for easy addition of other IDs
# in the future 

zeroing_col_2 <- "pco2_2_instrumentZero"
zeroing_col_3 <- "pco2_3_instrumentZero"
...
all_params <- c(lat_col, lon_col, sst_col, press_col, cond_col, co2_col_2, zeroing_col_2, co2_col_3, zeroing_col_3, other[[1]])   # Replaced "order"
...
remove_if_na <- c(lat_col, lon_col)           
...
tags_file <- fromJSON(paste(readLines("tag_details/Tag_details_E2M3A_pCO2-both.json"),collapse=""))
...
df <- read.table(output_df_all, header=TRUE, sep="\t", stringsAsFactors = FALSE)
...
new_df <- lapply(new_df, function(x) { x[x==""] <- NA ; x })
# Because of the empty strings?? pco2_2_CO2 is read as character ->  convert to numeric
new_df$pco2_2_CO2 <- as.double(new_df$pco2_2_CO2)
...
save(new_df, file="E2M3Araw_2015_2017.RData")
