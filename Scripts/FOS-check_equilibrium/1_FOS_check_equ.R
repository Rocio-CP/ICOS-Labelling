...
#---------------------------------------
# create subset containing only date, time and CO2. This will be used in the following script where measurements
# before equilibrium is reached.

#if(is.numeric(additional_CO2_col)){
#df_sub <- df[,c(ncol(df), CO2_col, additional_CO2_col)]
#} else {
#  df_sub <- df[,c(ncol(df), CO2_col)]
#}


# Write the new data frame out in new file.
#out_file <- paste(output_dir, "/", input_files[1], sep="")
#write.csv(df, file=out_file, row.names=FALSE, fileEncoding="UTF8")
