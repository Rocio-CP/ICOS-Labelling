...
colnames(new_df) <- colnames(df_sub)


while (data_finished == 0) {

current_diff <- abs(difftime(df_sub$date.time[current_row],df_sub$date.time[current_row+1],unit="secs"))

if(current_diff<time_limit){
  current_row <- current_row+1
} else {
    new_df[count,] <- df_sub[current_row,]
    new_df$date.time[count] <- toString(df_sub$date.time[current_row])
    count <- count + 1
    current_row <- current_row +1 
}

if(current_row == nrow(df_sub)-1) {
  data_finished <- 1
}
...
write.table(new_df, file=out_file, row.names=FALSE, fileEncoding="UTF8")
