################################################################################
### Converts raw FOS data from E2M3A to human readable format                ### 
################################################################################
...
#-----
# About E2M3A IDs (mail Stefano 2018-05-15):
#$SBE37PO: Sea-Bird SBE 37-SMP-ODO CTD @ 2m
#$PCO2-2: Pro Oceanus CO2-Pro @2m (S/N 29-103-45 )
#$PCO2-3: Pro Oceanus CO2-Pro @2m (S/N 34-215-45 )

#During the maintenance of the E2M3A buoy we switch the Pro Oceanus, and sometimes also the 
# SBE 37-SMP-ODO CTD with some other one.
...
# What is the order of measurements. Remember to include position (has to be string).
order <- c(co2, lat, lon, sst, press, cond, other[[1]])
...
# Import data frame template for E2M3A station
df <- read.table("templates/E2M3A_template.txt",header=T, fileEncoding="UTF8")
...
while (data_finished==0) {
  ...
  if (grepl("PCO2-2", string)) {
    df$timestamp[df_count] <- string_split[[1]][2]
    df$date[df_count] <- sprintf("%06d",as.numeric(string_split[[1]][3]))
    df$time[df_count] <- string_split[[1]][4]
    df$pCO2_2_instrumentDate[df_count] <- string_split[[1]][5]
    df$pCO2_2_instrumentTime[df_count] <- string_split[[1]][6]
    df$pCO2_2_instrumentZero[df_count] <- string_split[[1]][9]
    df$pCO2_2_instrumentMeasure[df_count] <- string_split[[1]][11]
    df$pCO2_2_CO2[df_count] <- string_split[[1]][14]
    df$pCO2_2_opticalCellTemp[df_count] <- string_split[[1]][16]
    df$pCO2_2_hum[df_count] <- string_split[[1]][18]
    df$pCO2_2_humTemp[df_count] <- string_split[[1]][20]
    df$pCO2_2_gasStreamPress[df_count] <- string_split[[1]][22]
  }
  
  
  if (grepl("PCO2-3", string)) {
    if(length(string_split[[1]])==15) {
    df$timestamp[df_count] <- string_split[[1]][2]
    df$date[df_count] <- sprintf("%06d",as.numeric(string_split[[1]][3]))
    df$time[df_count] <- string_split[[1]][4]
    df$pCO2_3_instrumentDateTime[df_count] <- string_split[[1]][5]
    df$pCO2_3_instrumentZero[df_count] <- string_split[[1]][7]
    df$pCO2_3_instrumentMeasure[df_count] <- string_split[[1]][8]
    df$pCO2_3_CO2[df_count] <- string_split[[1]][9]
    df$pCO2_3_opticalCellTemp[df_count] <- string_split[[1]][10]
    df$pCO2_3_hum[df_count] <- string_split[[1]][11]
    df$pCO2_3_humTemp[df_count] <- string_split[[1]][12]
    df$pCO2_3_gasStreamPress[df_count] <- string_split[[1]][13]
    df$pCO2_3_irgaEdtectorTemp[df_count] <- string_split[[1]][14]
    df$pCO2_3_irgaSourceTemp[df_count] <- string_split[[1]][15]
    }
  }
...
df <- read.table(output_df_all, header=TRUE, sep="\t", strip.white=TRUE)
...
for (i in 1:nrow(df)){
  ...
  if (!is.na(df$date[i])) {
    ...
    iso_time <- gsub('([0-9][0-9])([0-9][0-9])([0-9][0-9])','\\1:\\2:\\3',df$time[i])
...
parameters <- c(sst, press, cond, co2, other)
...
#------------------------------------------------------------------------------------
# COVENT TO FORMAT ACCEPTED BY QUINCE
# Need to move measurements from different sensors (with different IDs) but close enough
# time stamp to the same row.

# Will go through new_df and look for rows where the trigger_parameter is present (the mandartory 
...
while (new_df_finished == 0) {
  ...
  if (!is.na(new_df[[trigger_param]][new_df_count])) {
    quince_df[1,] <- NA     
    
    # Adds the limiting param measurement and date to the quince_df,
    quince_df[[trigger_param]] <- new_df[[trigger_param]][new_df_count]
    quince_df[[paste("datetime_",trigger_param,sep="")]] <- as.character(new_df$datetime[new_df_count])
      ...
      if (n!=trigger_param){
        ...
        while ((row_found == 0) && ((new_df_count+row_count) <= nrow(new_df))) {
          ...
          if (!is.na(new_df[[n]][new_df_count + row_count])) {            
            ...
            if (!is.na(new_df[[trigger_param]][new_df_count + row_count])) {
              row_found <- 1
            }         
          }            
        }
...
##----------
#PI_date_format <- "%d/%m/%Y %H:%M:%S"
#PI_date_col_name <- "date_time"
...
#
#PI_df$date_time <- as.POSIXct(PI_df$date_time,tz="UTC", format=PI_date_format)
#
#
#co2_compare_image_name <- paste("output/5.co2_compare_plot", input_files[1], "png", sep=".")
#png(co2_compare_image_name, width=1000)
#  
#plot(PI_df[[PI_date_col_name]], PI_df[[PI_co2_col_name]], ylab="CO2", xlab="Time")
