...
##-----------------------------------------------------------------------------
  ...
  # Loop through the input files
      ...
      # Manually edit yrange for plot:
        ...
        plot (data$date.time, zeros, col="black", lwd=2, xlab="Date", ylab="CO2 amonaly [unit]", type="l", ylim = c(yrange[1], yrange[2]))

	    for (k in 1:length(std_names)) {
            lines(df_list[[k]]$date.time, df_list[[k]]$diff, col=color[k])
		}
  
	    legend("topleft", legend = c("STD1", "STD2", "STD3", "STD4", "Base line"), col=c(color,"black"), lwd=c(1,1,1,1,2))
	    dev.off()
